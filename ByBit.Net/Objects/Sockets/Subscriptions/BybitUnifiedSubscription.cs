...
{
    internal class BybitUnifiedSubscription<T> : Subscription<BybitOptionsQueryResponse, BybitOptionsQueryResponse>
    {
        private string[] _topics;
        private Action<DataEvent<T>> _handler;
        public override HashSet<string> ListenerIdentifiers { get; set; }

        public BybitUnifiedSubscription(ILogger logger, string[] topics, Action<DataEvent<T>> handler, bool auth = false) : base(logger, auth)
        {
            _topics = topics;
            _handler = handler;
        ...
        {
            return new BybitUnifiedQuery("subscribe", _topics);
        }
        public override Query? GetUnsubQuery()
        {
            return new BybitUnifiedQuery("unsubscribe", _topics);
        }
    }
}

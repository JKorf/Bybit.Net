...
namespace Bybit.Net.Interfaces.Clients.InverseFuturesApi
{
    ...
    public interface IBybitClientInverseFuturesApiAccount
    {
        ...
        /// <param name="coin">Currency alias. Required if not passing symbol</param>
        /// <param name="hedgeMode">Hedgemode</param>
        /// <param name="receiveWindow">The receive window for which this request is active. When the request takes longer than this to complete the server will reject the request</param>
        /// <param name="ct">Cancellation token</param>
        /// <returns></returns>
        Task<WebCallResult> SetPositionModeAsync(string symbol, string coin, bool hedgeMode, long? receiveWindow = null, CancellationToken ct = default);

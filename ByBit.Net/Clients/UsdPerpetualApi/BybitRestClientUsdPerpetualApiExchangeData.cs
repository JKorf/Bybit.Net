...
{
    /// <inheritdoc />
    public class BybitRestClientUsdPerpetualApiExchangeData : IBybitRestClientUsdPerpetualApiExchangeData
    {
        private BybitRestClientUsdPerpetualApi _baseClient;

        internal BybitRestClientUsdPerpetualApiExchangeData(BybitRestClientUsdPerpetualApi baseClient)
        {
        ...
        /// <inheritdoc />
        public async Task<WebCallResult<IEnumerable<BybitTrade>>> GetTradeHistoryAsync(string symbol, int? limit = null, CancellationToken ct = default)
        {
            var parameters = new Dictionary<string, object>()
            {
                { "symbol", symbol }

...
{
    /// <inheritdoc cref="IBybitSocketClientCopyTradingApi" />
    public class BybitSocketClientCopyTradingApi : SocketApiClient, IBybitSocketClientCopyTradingApi
    {
        internal BybitSocketClientCopyTradingApi(ILogger log, BybitSocketOptions options)
            : base(log, options.Environment.SocketBaseAddress, options, options.CopyTradingOptions)
        {
            ContinueOnQueryResponse = true;
            UnhandledMessageExpected = true;
            KeepAliveInterval = TimeSpan.Zero;

            SendPeriodic("Ping", options.CopyTradingOptions.PingInterval, (connection) =>
            {
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToPositionUpdatesAsync(Action<DataEvent<IEnumerable<BybitCopyTradingPositionUpdate>>> handler, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken>>(data =>
            {
                ...
                if (!desResult)
                {
                    _logger.Log(LogLevel.Warning, $"Failed to deserialize {nameof(BybitCopyTradingPositionUpdate)} object: " + desResult.Error);
                    return;
                }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToUserTradeUpdatesAsync(Action<DataEvent<IEnumerable<BybitCopyTradingUserTradeUpdate>>> handler, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken>>(data =>
            {
                ...
                if (!desResult)
                {
                    _logger.Log(LogLevel.Warning, $"Failed to deserialize {nameof(BybitCopyTradingUserTradeUpdate)} object: " + desResult.Error);
                    return;
                }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToOrderUpdatesAsync(Action<DataEvent<IEnumerable<BybitCopyTradingOrderUpdate>>> handler, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken>>(data =>
            {
                ...
                if (!desResult)
                {
                    _logger.Log(LogLevel.Warning, $"Failed to deserialize {nameof(BybitCopyTradingOrderUpdate)} object: " + desResult.Error);
                    return;
                }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToBalanceUpdatesAsync(Action<DataEvent<BybitCopyTradingBalanceUpdate>> handler, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken>>(data =>
            {
                ...
                if (!desResult)
                {
                    _logger.Log(LogLevel.Warning, $"Failed to deserialize {nameof(BybitCopyTradingBalanceUpdate)} object: " + desResult.Error);
                    return;
                }

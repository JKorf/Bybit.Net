...
namespace Bybit.Net.Clients.DerivativesApi.UnifiedMarginApi
{
    ...
    public class BybitSocketClientUnifiedMarginStreams : SocketApiClient, IBybitSocketClientUnifiedMarginStreams
    {
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToPositionUpdatesAsync(Action<DataEvent<IEnumerable<BybitUnifiedMarginPositionUpdate>>> handler, CancellationToken ct = default)
        {
            ...
            return await SubscribeAsync(new BybitDerivativesRequestMessage() { Operation = "subscribe", CustomisedId = Guid.NewGuid().ToString(), Parameters = new[] { "user.position.unifiedAccount" } },
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToUserTradeUpdatesAsync(Action<DataEvent<IEnumerable<BybitDerivativesUserTradeUpdate>>> handler, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken>>(data =>
            {
                ...
                if (!desResult)
                {
                    _log.Write(LogLevel.Warning, $"Failed to deserialize {nameof(BybitDerivativesUserTradeUpdate)} object: " + desResult.Error);
                    return;
                }
            ...
            return await SubscribeAsync(new BybitDerivativesRequestMessage() { Operation = "subscribe", CustomisedId = Guid.NewGuid().ToString(), Parameters = new[] { "user.execution.unifiedAccount" } },
                null, true, internalHandler, ct).ConfigureAwait(false);
        }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToOrderUpdatesAsync(Action<DataEvent<IEnumerable<BybitUnifiedMarginOrderUpdate>>> handler, CancellationToken ct = default)
        {
            ...
            return await SubscribeAsync(new BybitDerivativesRequestMessage() { Operation = "subscribe", CustomisedId = Guid.NewGuid().ToString(), Parameters = new[] { "user.order.unifiedAccount" } },
                null, true, internalHandler, ct).ConfigureAwait(false);
        }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToBalanceUpdatesAsync(Action<DataEvent<BybitUnifiedMarginBalance>> handler, CancellationToken ct = default)
        {
            ...
            return await SubscribeAsync(new BybitDerivativesRequestMessage() { Operation = "subscribe", CustomisedId = Guid.NewGuid().ToString(), Parameters = new[] { "user.wallet.unifiedAccount" } },
                null, true, internalHandler, ct).ConfigureAwait(false);
        }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToGreeksUpdatesAsync(Action<DataEvent<IEnumerable<BybitGreeksUpdate>>> handler, CancellationToken ct = default)
        {
            ...
            return await SubscribeAsync(new BybitDerivativesRequestMessage() { Operation = "subscribe", CustomisedId = Guid.NewGuid().ToString(), Parameters = new[] { "user.greeks.unifiedAccount" } },
                null, true, internalHandler, ct).ConfigureAwait(false);
        }

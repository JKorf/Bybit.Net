...
{
    /// <inheritdoc />
    public class BybitRestClientInverseFuturesApiAccount : IBybitRestClientInverseFuturesApiAccount
    {
        private BybitRestClientInverseFuturesApi _baseClient;

        internal BybitRestClientInverseFuturesApiAccount(BybitRestClientInverseFuturesApi baseClient)
        {
        ...
        /// <inheritdoc />
        public async Task<WebCallResult<IEnumerable<BybitPosition>>> GetPositionAsync(string symbol, long? receiveWindow = null, CancellationToken ct = default)
        {
            var parameters = new Dictionary<string, object>()
            {
                { "symbol", symbol }
        ...
        public async Task<WebCallResult> SetPositionModeAsync(string symbol, string asset, bool hedgeMode, long? receiveWindow = null, CancellationToken ct = default)
        {
            ...
            parameters.Add("mode", hedgeMode ? 3 : 0);
            parameters.AddOptionalParameter("recv_window", receiveWindow?.ToString(CultureInfo.InvariantCulture) ?? _baseClient.ClientOptions.ReceiveWindow.TotalMilliseconds.ToString(CultureInfo.InvariantCulture));

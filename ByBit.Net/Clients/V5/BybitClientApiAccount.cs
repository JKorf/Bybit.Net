...
{
    /// <inheritdoc />
    public class BybitClientApiAccount : IBybitClientApiAccount
    {
        ...
        /// <inheritdoc />
        public async Task<WebCallResult<BybitResponse<BybitAccountInfo>>> GetMarginAccountInfoAsync(CancellationToken ct = default)
        {
            var parameters = new Dictionary<string, object>();
            return await _baseClient.SendRequestAsync<BybitResponse<BybitAccountInfo>>(_baseClient.GetUrl("v5/account/info"), HttpMethod.Get, ct, parameters, true).ConfigureAwait(false);
        }

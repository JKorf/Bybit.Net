...
namespace Bybit.Net.Clients.V5
{
    ...
    public class BybitSocketClientSpotStreams : BybitSocketClientBaseStreams, IBybitSocketClientSpotStreams
    {
        internal BybitSocketClientSpotStreams(Log log, BybitSocketClientOptions options)
            : base(log, options, "/spot")
        {
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToLeveragedTokenTickerUpdatesAsync(IEnumerable<string> symbols, Action<DataEvent<BybitLeveragedTokenTicker>> handler, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken>>(data =>
            {
                ...
                if (!desResult)
                {
                    _log.Write(LogLevel.Warning, $"Failed to deserialize {nameof(BybitLeveragedTokenTicker)} object: " + desResult.Error);
                    return;
                }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToLeveragedTokenNavUpdatesAsync(IEnumerable<string> symbols, Action<DataEvent<BybitLeveragedTokenNav>> handler, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken>>(data =>
            {
                ...
                if (!desResult)
                {
                    _log.Write(LogLevel.Warning, $"Failed to deserialize {nameof(BybitLeveragedTokenNav)} object: " + desResult.Error);
                    return;
                }
